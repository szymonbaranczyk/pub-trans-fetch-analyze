...
import api.{LocationFetchControlApi, ErrorResponseException, Marshalling}
 ...
 * to add new api (eg. FooApi) You need to add it to routes
 *   val routes =
 *   new LocationApi().route ~
 *   new FooApi().route
 *
 * see `~` sign
 */
trait Api extends RouteConcatenation {
  ...
  val routes =
    new LocationFetchControlApi().route
